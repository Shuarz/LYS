...
int ePelicula_hardcode(ePelicula listadoPelicula[], int limite)
{
    ...
    char nacionalidad[5][TAMNACIONALIDAD] = {"Argentina","China","Mexico","Canada","Italia"};
    ...
    if(listaEstado == 0)
    {
        if(limite > 0 && listadoPelicula != NULL)
        {
            ...
            if(busqueda >= 0 && busqueda < limite)
            {
                ...
                printf("\nEsta Seguro que quiere dar de baja (y): ");
                fflush(stdin);
                opcion = getchar();
...
void ePelicula_ingresarAnio(ePelicula* pelicula, char mensaje[], int modificacion)
{
    ...
    if(modificacion)
    {
        modificacionFlag = ePelicula_confirmacion("\nEstas Seguro de cambiar el anio de estreno? (y): ", 'y');
        if(modificacionFlag == -1)
        {
            printf("Saliendo...\n");
        }
